...
public class MatchTeam
{
    ...
    public void CalcAttrib()
    {
        ...
        // Console.WriteLine("dr ", Attrib.Dribbling);
    }
            ...
            case PassType.LoseTime:
                {
                    return GetOutcome("lose time",
                        (true, 1200 + (float)Math.Pow(25f * player.Attrib.Passing, 1.67f)),
                        (false, 300 + (float)Math.Pow(Opp.Attrib.Pace + Opp.Attrib.Marking * 2f, 1.25f))
                    );
                }
            ...
            case PassType.Advance:
                {
                    return GetOutcome("pass",
                        (true, 1200 + (float)Math.Pow(20f * player.Attrib.Passing * 8f, 1.67f)),
                        (false, 300 + (float)Math.Pow(Opp.Attrib.Pace + Opp.Attrib.Marking, 1.25f))
                    );
                }
            ...
            case PassType.ThroughBall:
                {
                    return GetOutcome("through ball",
                        (true, 200 + (float)Math.Pow(15f * player.Attrib.Passing, 1.6f)),
                        (false, 300 + (float)Math.Pow(Opp.Attrib.Pace * 2f + Opp.Attrib.Marking, 1.25f))
                    );
                }
            ...
            case PassType.Clear:
                {
                    return GetOutcome("clear",
                        (true, 200 + (float)Math.Pow(15f * player.Attrib.Passing, 1.6f)),
                        (false, 300 + (float)Math.Pow(Opp.Attrib.Pace * 1f + Opp.Attrib.Marking, 0.65f))
                    );
                }
        }
    ...
    float GetPassAdvance(MatchSlot player, PassType type)
    {
        switch (type)
        {
            case PassType.LoseTime:
                {
                    return RNG.RandfRange(-0.03f, 0.03f) * passMx;
                }

            case PassType.Advance:
                {
                    return RNG.RandfRange(0.08f, 0.2f) * 1.2f * passMx;
                }

            case PassType.ThroughBall:
                {
                    return RNG.RandfRange(0.15f, 0.3f) * passMx;
                }

            case PassType.Clear:
                {
                    return RNG.RandfRange(0.2f, 0.35f) * passMx;
                }
        }
    ...
    void ResolvePass(PassType type)
    {
        ...
        var playersByPassingChance = PlayingSlots.ToDictionary(s => s, s => (float)Math.Pow(2 + s.Attrib.Passing * s.Ptc, 2.25f));
        ...
        {
            var playersByMarkingAbility = Opp.PlayingSlots.ToDictionary(s => s, s => (float)Math.Pow((5 + s.Attrib.Marking * 1.25f + s.Attrib.Pace) * s.Ptc, 2.25f));
    ...
    void ResolveDribble()
    {
        var playersByPassingChance = PlayingSlots.ToDictionary(s => s, s => (float)Math.Pow(2 + s.Attrib.Dribbling * s.Ptc, 2.25f));
        var oppPlayersByMarkingAbility = Opp.PlayingSlots.ToDictionary(s => s, s => (float)Math.Pow(2 + s.Attrib.Marking * s.Ptc, 2.25f));
        ...
        var outcome = GetOutcome("dribbled past",
            (MatchTeamDribbleOutcome.Success, (float)Math.Pow(whoDribbled.Attrib.Dribbling, 1.25f)),
            (MatchTeamDribbleOutcome.OppTackle, (float)Math.Pow((whoMarked.Attrib.Marking + whoMarked.Attrib.Pace) * 0.7f, 1.1f)),
            (MatchTeamDribbleOutcome.GetFouled, (float)Math.Pow((15f - whoMarked.Attrib.Mentality) * 0.25f, 1.2f))
        );
        ...
        switch(outcome)
        {
            case MatchTeamDribbleOutcome.Success:
                {
                    var value = RNG.RandfRange(0.1f, 0.2f) * (float)Math.Pow(whoDribbled.Attrib.Pace, 1.5f) * 0.1f;
    ...
    void ResolveFoul(MatchSlot whoWon, MatchSlot whoCommitted)
    {
        ...
        var foulOutcome = GetOutcome("dribbled past",
            (MatchTeamFoulOutcome.None, (float)Math.Pow(whoWon.Attrib.Mentality * 3f + whoCommitted.Attrib.Pace + whoCommitted.Attrib.Marking, 1.25f)),
            (MatchTeamFoulOutcome.YellowCard, (float)Math.Pow(whoWon.Attrib.Mentality * 3f + whoCommitted.Attrib.Pace + whoCommitted.Attrib.Marking, 1.25f) * 0.75f - (whoCommitted.CardState == MatchSlotCardState.Y ? 0.7f : 0f)),
            (MatchTeamFoulOutcome.RedCard, (float)Math.Pow(whoWon.Attrib.Mentality * 3f + whoCommitted.Attrib.Pace + whoCommitted.Attrib.Marking, 1.25f) * 0.05f)
        );
    ...
    void ResolveHold()
    {
        var held = GetOutcome("hold",
            (true, 5000),
            (false, 300 + (float)Math.Pow(Opp.Attrib.Pace, 1.25f))
        );
    ...
    {
        var prev = Match.Ball;
        Match.Ball = Math.Clamp(Match.Ball + value * (IsHome ? 1f : -1f), 0, 1);
    ...
    void ResolveAttack(bool isHeader, MatchSlot whoShot = null)
    {
        ...
        if (whoShot == null)
        {
            var playersByAerial = PlayingSlots.ToDictionary(s => s, s => (float)Math.Pow(2 + s.Attrib.Aerial * s.Ptc, 2.25f));
            ...
            case MatchTeamFinishOutcome.Block:
                {
                    ...
                    var playersByMarking = Opp.PlayingSlots.ToDictionary(s => s, s => (float)Math.Pow(2 + s.Attrib.Marking * s.Attrib.Pace * s.Ptc, 2.25f));
    ...
    void ResolveAerial()
    {
        ...
        var aerialOutcome = GetOutcome("aerial",
            (MatchTeamAerialOutcome.Win, (float)Math.Pow(myJumper.Attrib.Aerial, 2f)),
            (MatchTeamAerialOutcome.Lose, (float)Math.Pow(oppJumper.Attrib.Aerial, 2f)),
            (MatchTeamAerialOutcome.GetFouled, 2 + (float)Math.Pow(Math.Max(0, myJumper.Attrib.Mentality - myJumper.Attrib.Mentality * 0.35f) * 6f, 1.4f))
        );
    ...
    MatchSlot GetPlayerFromList(string otcof, Dictionary<MatchSlot, float> ocs)
    {
        ...
        var odd = RNG.RandiRange(0, combinedWeight);
        ...
        {
            var chance = Math.Round((float)p.Value / combinedWeight * 100f);
